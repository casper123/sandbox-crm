...
class Productsmodel extends CI_Model {
    ...
    function get_tinpack(){
     ...
     function get_record_page_wise($currentpage = 1, $menu_id = ""){
        ...
        if($menu_id != "" && intval($menu_id) > 0){
         ...
         $query = $this->db->query("SELECT *, (SELECT `name` FROM `menu` WHERE `menu`.`id` = `products`.`menu_id` )  AS menu_name FROM  " . $this->tablename." WHERE products.`product_type` != 'tinpack' ".$concate);
        ...
        $query = $this->db->query("SELECT *, (SELECT `name` FROM `menu` WHERE `menu`.`id` = `products`.`menu_id` )  AS menu_name FROM  " . $this->tablename." WHERE products.`product_type` != 'tinpack' ".$concate."   Limit ".$offset.",".$limit);
    ...
    function get_product_price_and_img($prod_id){
        $query = $this->db->query("SELECT price,product_image FROM  " . $this->tablename." WHERE id=".intval($prod_id));
